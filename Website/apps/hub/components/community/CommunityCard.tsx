...
import { CommunityPerson, CommunityEvent } from '../../types';
import { Manifest } from '../../services/contentService';
import GitHubImage from '../GitHubImage';
import MarkdownRenderer from '../MarkdownRenderer';

type CommunityItem = CommunityPerson | CommunityEvent;
...
const CommunityCard: React.FC<CommunityCardProps> = ({ item, manifest, onViewDetails }) => {
  return (
    <div 
    ...
    >
      <div className="overflow-hidden relative">
        <GitHubImage
            ...
            placeholderCat="community"
        />
        <div className="absolute top-2 right-2 px-2 py-1 bg-gray-900/70 backdrop-blur-sm text-amber-200 text-xs font-semibold rounded-full">
          {item.type}
        </div>
      </div>
      <div className="flex-grow flex flex-col p-4">
        ...
        <div className="text-sm text-gray-300 flex-grow my-2">
            <MarkdownRenderer markdown={item.desc} className="prose-sm" />
        </div>
        <button 
